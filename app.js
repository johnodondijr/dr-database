...
// ══════════════════════════════════════════════════════════
// STAFF ACCOUNTS (local auth — no Supabase auth needed)
// ══════════════════════════════════════════════════════════
...
let proDB         = [];
let lbDB          = [];
let allDocs       = {};   // { "pro_1": { "Passport": "https://..." } }
let allTimelines  = {};   // { "pro_1": [{action,user,ts}] }
...
let docsTarget    = null; // { type, id, name }
const PER_PAGE    = 20;
...
function doLogin() {
  ...
  document.getElementById('user-chip').textContent      = account.display;
  loadAllData();
}
  ...
  try {
    ...
    if (tlRes.data) {
      tlRes.data.forEach(row => {
        allTimelines[row.key] = row.entries;
      });
    }

    // Custom stages from settings
    if (stagesRes.data) {
...
// Convert Excel serial date numbers to ISO strings; leave text labels (e.g. "CV SELECTION") as-is
function normalizeDateField(v) {
  if (v === '' || v === null || v === undefined) return null;
  if (typeof v === 'number') return xlToISO(v);
  return v; // text labels like "CV SELECTION", "INTERVIEW", or already-ISO strings
}
...
async function seedProData() {
  const seed = JSON.parse(JSON.stringify(PRO_SEED)).map(r => {
    ...
    delete r.id; // let Supabase auto-generate
    return r;
  });
...
function renderTimelineHTML(type, id) {
  ...
  return items.map(item => {
    ...
    return `<div class="tl-item">
      ...
      <div><div class="tl-action">${item.action}</div>
      <div class="tl-meta">${item.user} · ${ds} ${ts}</div></div>
    </div>`;
  }).join('');
}
...
function getRefundStatus(r) {
  if (r.ppStatus === 'HAD PP' || r.pp_status === 'HAD PP') return 'N/A';
...
function isInProcessLB(r) {
  ...
  return pp !== 'HAD PP' && ts === 'NOT YET';
}
// normalize field names (handle both camelCase seed and snake_case Supabase)
function norm(r, camel, snake) { return r[camel] !== undefined ? r[camel] : r[snake]; }
...
function rebuildStageSelects() {
  ...
  if (proFilter) {
    const cur = proFilter.value;
    proFilter.innerHTML = '<option value="">All stages</option>' +
      proStages.map(s => `<option value="${s}">${s}</option>`).join('');
    proFilter.value = cur;
  }
  const lbFilter = document.getElementById('lb-travel-f');
  if (lbFilter) {
    const cur = lbFilter.value;
    lbFilter.innerHTML = '<option value="">All travel statuses</option>' +
...
function renderDash() {
  ...
  document.getElementById('dash-pro-card').innerHTML = `
    ...
    <div class="dash-row"><span class="dash-row-label">In process <small style="color:var(--text-3)">(OL → Visa)</small></span><span class="dash-row-val" style="color:var(--amber)">${proInProcess}</span></div>
  ...
  // LB summary — refund balance ONLY from TRAVELLED candidates
  ...
  document.getElementById('dash-lb-card').innerHTML = `
    ...
    <div class="dash-row"><span class="dash-row-label">In process <small style="color:var(--text-3)">(PP applied, not yet travelled)</small></span><span class="dash-row-val" style="color:var(--amber)">${lbInProcess}</span></div>
  ...
  // Pro stage breakdown bar chart
  const maxS = Math.max(...proStages.map(s => proDB.filter(r => r.stage === s).length), 1);
  document.getElementById('dash-pro-stages').innerHTML = `<h3>💼 Stage Breakdown</h3>` +
    proStages.map(s => {
  ...
  const lbNotYet     = lbDB.filter(r => (r.travelStatus||r.travel_status) === 'NOT YET').length;
  const lbNotTrav    = lbDB.filter(r => (r.travelStatus||r.travel_status) === 'NOT TRAVELLED').length;
  document.getElementById('dash-lb-refunds').innerHTML = `<h3>🏠 Refund Overview</h3>
    <div class="dash-row"><span class="dash-row-label">✅ Refund complete</span><span class="dash-row-val" style="color:var(--green)">${lbComplete}</span></div>
    ...
    <div class="dash-row"><span class="dash-row-label">↩️ Returned</span><span class="dash-row-val" style="color:var(--red)">${lbReturned}</span></div>
    <div class="dash-row"><span class="dash-row-label">N/A (HAD PP)</span><span class="dash-row-val">${lbNA}</span></div>
    <div class="dash-row"><span class="dash-row-label">🕐 Not yet travelled</span><span class="dash-row-val" style="color:var(--blue)">${lbNotYet}</span></div>
    <div class="dash-row"><span class="dash-row-label">❌ Did not travel</span><span class="dash-row-val">${lbNotTrav}</span></div>`;
}
...
function renderPro() {
  ...
  document.getElementById('pro-metrics').innerHTML = `
    ...
    <div class="metric"><div class="metric-label">In process</div><div class="metric-val amber">${proDB.filter(isInProcessPro).length}</div></div>
    <div class="metric"><div class="metric-label">Travelled</div><div class="metric-val green">${proDB.filter(r=>r.stage==='TRAVELLED').length}</div></div>
  ...
  csel.innerHTML = '<option value="">All companies</option>' + companies.map(c => `<option value="${c}"${c===ccur?' selected':''}>${c}</option>`).join('');
  ...
  if (!slice.length) {
    tbody.innerHTML = `<tr><td colspan="13"><div class="empty">No candidates found</div></td></tr>`;
  } else {
...
function openProForm() {
  ...
  document.getElementById('pro-tab-pipeline').style.display = 'none';
  document.getElementById('pro-tab-commission').style.display = 'none';
  document.getElementById('pro-tab-timeline').style.display = 'none';
...
function editPro(id) {
  ...
  document.getElementById('pro-tab-details').style.display = '';
  ['pipeline','commission','timeline'].forEach(t => document.getElementById(`pro-tab-${t}`).style.display='none');
  document.getElementById('pro-modal').querySelectorAll('.modal-tab').forEach((b,i) => b.classList.toggle('active', i===0));
  document.getElementById('pro-modal').classList.add('open');
}
...
async function savePro() {
  ...
  if (editingProId) {
    ...
    rec.id = Date.now(); // temp id until Supabase returns real one
    proDB.push(rec);
...
function renderLB() {
  ...
  document.getElementById('lb-metrics').innerHTML = `
    ...
    <div class="metric"><div class="metric-label">In process</div><div class="metric-val amber">${lbDB.filter(isInProcessLB).length}</div></div>
    <div class="metric"><div class="metric-label">Travelled</div><div class="metric-val green">${lbDB.filter(r=>(r.travelStatus||r.travel_status)==='TRAVELLED').length}</div></div>
    <div class="metric"><div class="metric-label">Refund balance</div><div class="metric-val sm amber">$${lbOwed-lbPaid}</div></div>
    <div class="metric"><div class="metric-label">Incomplete</div><div class="metric-val red">${lbDB.filter(r=>getRefundStatus(r)==='incomplete').length}</div></div>`;
